var Ohnishj = (function() {
  ...
  function findVerbSplit(tokens, wordIndices) {
      ...
      if (inSubject) {
        ...
        if (subjectWordCount > 1 && !subjectStarters.test(word)) {
          if (i + 1 < wordIndices.length) {
            const nextIdx = wordIndices[i + 1];
            const nextWord = tokens[nextIdx].value.toLowerCase().replace(/[^a-z]/g, '');
    ...
    let splitPoint = Math.floor(wordIndices.length * 0.4);
